...
import com.michael.aplicationTodoList.repository.TaskRepository;
import com.michael.aplicationTodoList.utils.Utils;
...
public class TaskController {
    ...
    public ResponseEntity create(@RequestBody TaskModel taskModel, HttpServletRequest request) {
        ...
        if (currentDate.isAfter(taskModel.getStarAt()) || currentDate.isAfter(taskModel.getEndAt())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("The start date and the end date cannot be earlier than the current date.");
        }
    ...
    public ResponseEntity update(@RequestBody TaskModel taskModel, @PathVariable UUID id, HttpServletRequest request) {
        ...
        if (!task.getIdUser().equals(idUser)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("The user does not have permission to edit this task.");
        }
